...
begin
  ...
  RandSeed := Seed; for i := 0 to (CacheItems - 1) do begin // Massive item extraction...
    ...
    if not(TSessionCache.Extract(Word(i), RequestHash, ClientAddress, ClientPort, SilentUpdate, CacheException)) then raise FailedUnitTestException.Create else TSessionCache.Delete(Word(i));
