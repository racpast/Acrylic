...
begin
  if ((ParamCount = 1) and (ParamStr(1) = '/?')) or (ParamCount <> 1) then begin
  ...
  if (ParamStr(1) = 'InstallAcrylicService') then begin
    ...
    CmdExec('NetSh.exe advfirewall firewall add rule name="Allow Acrylic DNS Proxy Service" dir=in action=allow program="' + MakeAbsolutePath('AcrylicService.exe') + '"');
  ...
  if (ParamStr(1) = 'UninstallAcrylicService') then begin
    ...
    CmdExec('NetSh.exe advfirewall firewall delete rule name="Allow Acrylic DNS Proxy Service"');
