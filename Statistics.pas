...
begin
  if TStatistics_Changed then begin
    ...
    for DnsIndex := 0 to (MAX_NUM_DNS_SERVERS - 1) do begin
      if (TStatistics_TotalResponsesReceivedFromDns[DnsIndex] > 0) then Data := Data + 'TotalResponsesReceivedFromDns' + IntToStr(DnsIndex + 1) + '        : ' + IntToStr(TStatistics_TotalResponsesReceivedFromDns[DnsIndex]) + #13#10 + 'MeanResponseTimeOfDns' + IntToStr(DnsIndex + 1) + 'mSeconds        : ' + FormatCurr('0.0', 1000.0 * (TStatistics_TotalFlyTimesMeasuredFromDns[DnsIndex] / TStatistics_TotalResponsesReceivedFromDns[DnsIndex])) + #13#10 else Data := Data + 'TotalResponsesReceivedFromDns' + IntToStr(DnsIndex + 1) + '        : ' + IntToStr(TStatistics_TotalResponsesReceivedFromDns[DnsIndex]) + #13#10 + 'MeanResponseTimeOfDns' + IntToStr(DnsIndex + 1) + 'mSeconds        : ?' + #13#10;
    end;
